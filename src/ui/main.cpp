...
//    - Page::IntegrationNumerique : calcul d'intégrales (Trapèze / Simpson)
...
char  in_expr[512]      = "x^2";
double in_a             = 0.0;
double in_b             = 1.0;
int   in_n              = 10;
int   in_methode_idx    = 0;
ResultatIntegration in_result = {};
bool  in_has_result     = false;
bool  in_show_table     = true;
...
static ItemNav nav[] = {
    ...
    { "  Intégration Numérique ", Page::IntegrationNumerique},
...
// Carte Intégration Numérique
{
    ImGui::PushStyleColor(ImGuiCol_ChildBg, Theme::BgCard);
    ImGui::BeginChild( "##c_in ", {card_w, card_h}, false);
    ImGui::SetCursorPos({16.0f, 14.0f});
    ImGui::TextColored(Theme::Accent,  "Intégration Numérique ");
    ImGui::SetCursorPos({16.0f, 36.0f});
    ImGui::TextColored(Theme::TextMuted,
         "Calcul d'aires par méthodes des trapèzes et Simpson. ");
    ImGui::SetCursorPos({16.0f, 54.0f});
    ImGui::TextColored(Theme::TextMuted,  "Résultat + Tableau des étapes ");
    ImGui::SetCursorPos({16.0f, card_h - 32.0f});
    if (BoutonAccent( "Ouvrir##in ", {card_w - 32.0f, 26.0f}))
        g_etat.page_courante = Page::IntegrationNumerique;
    ImGui::EndChild();
    ImGui::PopStyleColor();
}
ImGui::SameLine(0, GAP);
ImGui::Dummy({card_w, card_h}); // Espace vide pour aligner la ligne
...
//  INTEGRATION NUMERIQUE
// =============================================================================
static void RenderIntegrationNumerique(float cw, float ch) {
const float GAP   = 16.0f;
const float LW    = cw * 0.38f;
const float RW    = cw - LW - GAP;
const float COL_H = ch - 80.0f;
const float IP    = 16.0f;
ImGui::TextColored(Theme::TextPrimary, "Intégration Numérique");
ImGui::TextColored(Theme::TextMuted, "Calcul d'intégrales définies par les méthodes des trapèzes et de Simpson.");
ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 12.0f);
ImGui::Separator();
ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 16.0f);
...
// Panneau Paramètres
ImGui::PushStyleColor(ImGuiCol_ChildBg, Theme::BgCard);
ImGui::BeginChild("##in_p", {LW, COL_H}, false);
{
    ImGui::SetCursorPos({IP, IP});
    ImGui::TextColored(Theme::TextMuted, "PARAMÈTRES");
    ImGui::SetCursorPosX(IP);
    ...
    ImGui::SetCursorPosX(IP);
    ImGui::SetNextItemWidth(LW - IP*2);
    ImGui::InputText("##in_expr", g_etat.in_expr, sizeof(g_etat.in_expr));
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10.0f);

    ImGui::SetCursorPosX(IP);
    if (ImGui::BeginTable("##inpt", 2, 0, {LW-IP*2, 0})) {
        ImGui::TableSetupColumn("l", ImGuiTableColumnFlags_WidthFixed, 130.0f);
        ImGui::TableSetupColumn("v", ImGuiTableColumnFlags_WidthStretch);
        LigneParam("Borne inférieure a");
        ImGui::SetNextItemWidth(-1);
        ImGui::InputDouble("##ina", &g_etat.in_a, 0.1, 1.0, "%.4f");
        LigneParam("Borne supérieure b");
        ImGui::SetNextItemWidth(-1);
        ImGui::InputDouble("##inb", &g_etat.in_b, 0.1, 1.0, "%.4f");
        LigneParam("Nombre d'intervalles n");
        ImGui::SetNextItemWidth(-1);
        ImGui::InputInt("##inn", &g_etat.in_n);
        LigneParam("Méthode");
        ImGui::SetNextItemWidth(-1);
        const char* methods[] = {"Trapeze", "Simpson"};
        ImGui::Combo("##inm", &g_etat.in_methode_idx, methods, IM_ARRAYSIZE(methods));
        ImGui::EndTable();
    }
    ...
    ImGui::SetCursorPosY(COL_H - IP - 36.0f);
    ImGui::SetCursorPosX(IP);
    if (BoutonAccent("  Calculer   ", {LW - IP*2, 36.0f})) {
        if (g_etat.in_a >= g_etat.in_b || g_etat.in_n <= 0) {
            g_etat.in_has_result = false;
        } else {
            if (g_etat.in_methode_idx == 0) {
                g_etat.in_result = trapeze(g_etat.in_expr, g_etat.in_a, g_etat.in_b, g_etat.in_n);
            } else {
                int n_calc = g_etat.in_n;
                if (n_calc % 2 != 0) n_calc++;
                g_etat.in_result = simpson(g_etat.in_expr, g_etat.in_a, g_etat.in_b, n_calc);
            }
            g_etat.in_has_result = true;
            char op[256];
            const char* meth_str = (g_etat.in_methode_idx == 0) ? "Trapèze" : "Simpson";
            snprintf(op, sizeof(op), "integration \"%s\" ; [%.4f, %.4f] ; n=%d ; %s",
                     g_etat.in_expr, g_etat.in_a, g_etat.in_b, g_etat.in_n, meth_str);
            HistoriqueAjouter("Intégration Numérique", op);
        }
    }
}
ImGui::EndChild();
...
// Panneau Résultat
ImGui::PushStyleColor(ImGuiCol_ChildBg, Theme::BgCard);
ImGui::BeginChild("##in_r", {RW, COL_H}, false);
{
    ImGui::SetCursorPos({IP, IP});
    ImGui::TextColored(Theme::TextMuted, "RÉSULTAT");
    ImGui::SetCursorPosX(IP);
    ...
    if (!g_etat.in_has_result) {
        const char* msg = "Entrez les paramètres puis cliquez Calculer.";
        float tw = ImGui::CalcTextSize(msg).x;
        ImGui::SetCursorPosY(COL_H * 0.38f);
        ImGui::SetCursorPosX(IP + (RW - IP*2 - tw)*0.5f);
        ImGui::TextColored(Theme::TextMuted, "%s", msg);
        ...
        if (g_etat.in_a >= g_etat.in_b || g_etat.in_n <= 0) {
             Badge("Paramètres invalides", Theme::Error);
             ImGui::SetCursorPosX(IP);
             ImGui::TextColored(Theme::TextMuted, "Vérifiez que a < b et n > 0.");
        } else {
            ResultatIntegration* r = &g_etat.in_result;
            Badge("Calcul terminé", Theme::Success);
            ImGui::SameLine();
            char buf[128];
            snprintf(buf, sizeof(buf), "  Résultat ≈ %.8f", r->resultat);
            ImGui::TextColored(Theme::TextPrimary, "%s", buf);
            ImGui::SetCursorPosX(IP);
            ImGui::TextColored(Theme::TextMuted, "Méthode : %s | Pas h = %.6f", r->methode, r->h);

            ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10.0f);
            ImGui::SetCursorPosX(IP);
            ImGui::Checkbox("Afficher le tableau des étapes", &g_etat.in_show_table);
            if (g_etat.in_show_table) {
                ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 8.0f);
                float th = COL_H - ImGui::GetCursorPosY() - IP;
                if (ImGui::BeginTable("##int", 5,
                        ImGuiTableFlags_Borders |
                        ImGuiTableFlags_RowBg |
                        ImGuiTableFlags_ScrollY |
                        ImGuiTableFlags_SizingFixedFit |
                        ...
                        {RW - IP, th})) {
                    ImGui::TableSetupScrollFreeze(0,1);
                    ImGui::TableSetupColumn("i", ImGuiTableColumnFlags_WidthFixed, 28.0f);
                    ImGui::TableSetupColumn("x_i", ImGuiTableColumnFlags_WidthStretch);
                    ImGui::TableSetupColumn("f(x_i)", ImGuiTableColumnFlags_WidthStretch);
                    ImGui::TableSetupColumn("Coeff.", ImGuiTableColumnFlags_WidthFixed, 50.0f);
                    ImGui::TableSetupColumn("Cumul", ImGuiTableColumnFlags_WidthStretch);
                    ImGui::TableHeadersRow();

                    int display_limit = (r->n > 500) ? 500 : r->n;
                    for (int i = 0; i <= display_limit; i++) {
                        ImGui::TableNextRow();
                        ImGui::TableNextColumn();
                        ImGui::TextColored(Theme::TextMuted, "%d", i);
                        ImGui::TableNextColumn();
                        ImGui::Text("%.6f", r->x[i]);
                        ImGui::TableNextColumn();
                        ImGui::Text("%.6f", r->fx[i]);
                        ImGui::TableNextColumn();
                        double coeff = 1.0;
                        if (i > 0 && i < r->n) {
                            if (strcmp(r->methode, "Simpson") == 0) {
                                coeff = (i % 2 != 0) ? 4.0 : 2.0;
                            } else {
                                coeff = 2.0;
                            }
                        ...
                        ImGui::Text("%.0f", coeff);
                        ImGui::TableNextColumn();
                        ImGui::Text("%.6f", r->sommes_cumulees[i]);
                    }
                    if (r->n > 500) {
                        ImGui::TableNextRow();
                        ImGui::TableNextColumn();
                        ImGui::TextColored(Theme::TextMuted, "...");
                        ImGui::TableNextColumn();
                        ImGui::TextColored(Theme::TextMuted, "... (%d points masqués)", r->n - 500);
                        for(int k=2; k<=4; k++) { ImGui::TableNextColumn(); ImGui::TextColored(Theme::TextMuted, "..."); }
                    }
                    ImGui::EndTable();
                }
            }
...
while (running) {
    ...
    switch (g_etat.page_courante) {
        ...
        case Page::EvaluateurFonction:
            ...
            break;
        case Page::IntegrationNumerique:
            if (font_titre) ImGui::PopFont();
            RenderIntegrationNumerique(cw, ch);
            break;
